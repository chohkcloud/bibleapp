...
export function SearchScreen({ navigation }: Props) {
  ...
  const handleSearch = async () => {
    ...
    try {
      // FTS5 검색 시도
      let searchResults = await bibleService.search(bibleVersion, trimmedQuery, language, 100);

      // FTS5 결과 없으면 단순 검색
      if (searchResults.length === 0) {
        searchResults = await bibleService.searchSimple(bibleVersion, trimmedQuery, language, 100);
      }
